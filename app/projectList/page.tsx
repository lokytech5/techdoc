...
const ProjectListPage = () => {
    const { data: projects, isLoading, isError, refetch } = useFetchUserProjects({ page: 1, limit: 10 });
    ...
    const handleConfirmDelete = () => {
        if (selectedProjectId) {
            ...
            removeProject.mutate(selectedProjectId, {
                onSuccess: () => {
                    setIsDeleting(false); // Set loading state to false on success
                    setModalOpen(false);
                    const updatedProjects = projects.filter(project => project.id !== selectedProjectId);
                    refetch(); // Refetch the projects
                },
                onError: () => {
                    setIsDeleting(false); // Set loading state to false on error
                }
            });
        }
    };
    ...
    return (
        <LayoutContainer>
            ...
            />
            {isDeleting && <LoadingSpinner />} {/* Display loading spinner while deleting */}
            <div className="p-5">
